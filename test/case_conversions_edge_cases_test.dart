...
void main() {
  ...
  group('CaseConversionsOnStringExt — acronyms & mixed case', () {
    ...
    // word ("XMLParser" -> ["XML","Parser"]). Acronyms followed by digits are
    // NOT split ("HTTP2" -> ["HTTP2"-as-one] in step 2; step 3 separates the
    // digit boundary, but the acronym itself stays whole).
    ...
    test('HTTP2 stays as one acronym, digit is split off', () {
      expect('HTTP2'.toSnakeCase(), equals('http_2'));
    });
  ...
  group('CaseConversionsOnStringExt — numbers & alphanumerics', () {
    test('digits adjacent to letters are split out as their own components',
        () {
      expect('version1'.toSnakeCase(), equals('version_1'));
      expect('v1'.toSnakeCase(), equals('v_1'));
      expect('1a'.toSnakeCase(), equals('1_a'));
      expect('1abc2'.toSnakeCase(), equals('1_abc_2'));
      expect('a1b2c3'.toSnakeCase(), equals('a_1_b_2_c_3'));
    });
    ...
    test('dotted version-like numbers split on the dot', () {
      ...
      expect('v1.2.3'.toSnakeCase(), equals('v_1_2_3'));
    });
  });
